...
myrand ran(1102, 1992122981, 617);
...
DataBase::DataBase(const string &username, const string &dictname) : username_(username)
{
    ...
    ifstream fin(("./record/database/" + filename_).c_str(), ios::in);
    wordlist_.clear();
    if (fin)
    {
        ReadHistory(fin);
...
bool DataBase::ReadDict(const string &dictname)
{
    ...
    if (!fin)
    {
        QMessageBox::critical(NULL, QObject::tr("错误"), QObject::tr("词典创建失败"));
        return false;
    }
...
DataBase::~DataBase()
{
    ofstream fout(("./record/database/" + filename_).c_str());
...
void DataBase::ChangeFeature(int num)
{
